...
class MockNotificationSender(
    ...
    private val metricsModule: NotificationMetricsModule,
) : NotificationSender {
    override suspend fun sendNotification(transfer: Transfer): SendResult {
        return metricsModule.measureSendNotificationRequest {
            sendCatching {
                webClient.post()
                    .bodyValue(
                        SendNotificationRequest(
                            ...
                            channels =
                                setOf(
                                    SendNotificationRequest.Channel.SMS,
                                    SendNotificationRequest.Channel.EMAIL,
                                ),
                        ),
                    )
                    .also { logger.info { "Sending notification request for transfer [${transfer.id}]" } }
                    .retrieve()
                    .toBodilessEntity()
                    .awaitSingle()
            }.onFailure {
